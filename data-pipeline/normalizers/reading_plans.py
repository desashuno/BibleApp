"""
reading_plans.py — Generates reading plan data for reading_plans, reading_plan_progress,
and reading_plan_entries tables.

These are not downloaded from external sources but generated from
the canonical Bible structure (66 books, 1189 chapters).

Output tables: reading_plans, reading_plan_progress, reading_plan_entries
"""
...
# Verse counts per chapter (book_number -> [verse_count_ch1, verse_count_ch2, ...])
# This is needed to compute end_verse_id accurately.
# Comprehensive list of verse counts per book per chapter (KJV versification).
VERSE_COUNTS: dict[int, list[int]] = {
    ...
    17: [22,23,15,17,14,14,10,17,32,3,17,8,30,16,24,10],
    ...
    20: [33,22,35,27,23,35,27,36,18,32,31,28,25,35,33,33,28,24,29,30,31],
    ...
    39: [14,17,18,6,24,15,17,16,44,25,19,6,18,14],
    ...
    46: [31,16,23,21,13,20,40,13,27,33,34,31,13,40,58,10],
...
def _build_daily_assignments(
    chapters: list[tuple[int, int]], days: int,
...
def _get_verse_count(book_num: int, chapter: int) -> int:
    """Get the number of verses in a given chapter."""
    vc = VERSE_COUNTS.get(book_num)
    if vc and 1 <= chapter <= len(vc):
        return vc[chapter - 1]
    return 1  # fallback


def _compute_verse_id(book_num: int, chapter: int, verse: int) -> int:
...
def normalize(raw_dir: Path, db: sqlite3.Connection) -> None:
    """Generate reading plans with entries."""
    ...
    plans = [
        ...
        },
        {
            ...
            "duration_days": 60,
            "type": "topical",
            "chapters": _chapters_for_books(range(18, 20)),
        },
        {
            ...
            "duration_days": 30,
            "type": "canonical",
            "chapters": _chapters_for_books(range(39, 43)),
        },
    ]
    ...
    for plan in plans:
        ...
        for day_num, day_chapters in enumerate(daily, start=1):
            ...
            # Insert reading plan entries with start/end verse IDs
            if day_chapters:
                first_book, first_ch = day_chapters[0]
                last_book, last_ch = day_chapters[-1]

                start_verse_id = _compute_verse_id(first_book, first_ch, 1)
                last_verse_count = _get_verse_count(last_book, last_ch)
                end_verse_id = _compute_verse_id(last_book, last_ch, last_verse_count)

                db.execute(
                    """INSERT INTO reading_plan_entries
                       (plan_id, day, start_verse_id, end_verse_id)
                       VALUES (?, ?, ?, ?)""",
                    (plan_uuid, day_num, start_verse_id, end_verse_id),
                )
                entry_count += 1
    ...
    db.commit()
    print(f"      ✓ Reading plans: {plan_count}")
    print(f"      ✓ Reading plan entries: {entry_count}")
