...
object OsisParser {
    ...
    private val JESUS_QUOTE_TAG = Regex("""<q\b[^>]*who\s*=\s*"Jesus"[^>]*>(.*?)</q>""", RegexOption.DOT_MATCHES_ALL)
    private val GENERIC_Q_TAG = Regex("""<q\b[^>]*>(.*?)</q>""", RegexOption.DOT_MATCHES_ALL)
    ...
    fun parse(xml: String, moduleName: String = "Unknown", abbreviation: String = "UNK"): ParseResult {
        ...
        for (match in VERSE_TAG.findAll(xml)) {
            ...
            val normalizedHtml = normalizeRedLetterMarkup(content.trim())
            val plainText = normalizedHtml
                .replace(NOTE_TAG, "")
                .replace(TAG_PATTERN, "")
                .trim()
    ...
    private fun normalizeRedLetterMarkup(rawContent: String): String {
        return rawContent
            .replace(JESUS_QUOTE_TAG) { match ->
                "<wj>${match.groupValues[1]}</wj>"
            }
            .replace(GENERIC_Q_TAG) { match ->
                match.groupValues[1]
            }
    }
